...
class _LockingSession(_QueuedSession):
    """Like _QueuedSession but raises OperationalError on the 3rd execute call.

    Use this to simulate a locked row (FOR UPDATE NOWAIT contention).
    Queue slot [1]=get_current_user, [2]=count query (passes), [3]=raises.
    """
    ...
    async def execute(self, query):
        self._call_count += 1
        if self._call_count == 3:  # 1st=get_current_user, 2nd=count, 3rd=asset NOWAIT
...
class _LockerLockingSession(_QueuedSession):
    """Raises OperationalError on locker lock (4th execute call)."""
...
class _LoanLockingSession(_QueuedSession):
    """Raises OperationalError on loan lock (5th execute call)."""
...
def test_checkout_returns_202_on_happy_path(client_with_overrides):
    ...
    # Queue: [1]=count(0), [2]=get_current_user, [3]=asset, [4]=locker
...
def test_checkout_returns_207_when_manager_send_command_returns_false(
    monkeypatch, client_with_overrides
):
    """If the WebSocket manager fails to send the `open_slot` command,
    the checkout should commit the DB state first, then return 207
...
def test_return_initiate_returns_207_when_manager_send_command_returns_false(
    monkeypatch, client_with_overrides
):
    """If the WebSocket manager fails to send the `open_slot` command,
    the return initiation should commit the DB state first, then return 207
    ...
    fake_db = _QueuedSession(
        student, [], SimpleNamespace(), asset, active_loan, active_loan, free_locker
    )
...
def test_checkout_returns_400_when_asset_not_available(client_with_overrides):
    ...
    fake_db = _QueuedSession(student, [], borrowed_asset, None, None)
    with client_with_overrides(fake_db) as client:
        response = client.post(
            "/api/v1/loans/checkout",
...
def test_checkout_returns_400_when_asset_has_no_locker(client_with_overrides):
    ...
    fake_db = _QueuedSession(student, [], orphaned_asset, None, None)
    with client_with_overrides(fake_db) as client:
        response = client.post(
            "/api/v1/loans/checkout",
...
def test_return_initiate_returns_202_on_happy_path(client_with_overrides):
    ...
    fake_db = _QueuedSession(
        student, [], SimpleNamespace(), asset, loan, loan, free_locker
    )
    with client_with_overrides(fake_db) as client:
        response = client.post(
            "/api/v1/loans/return/initiate",
...
def test_return_initiate_returns_403_for_wrong_user(client_with_overrides):
    ...
    fake_db = _QueuedSession(student_b, [], SimpleNamespace(), asset, loan)
    with client_with_overrides(fake_db) as client:
        response = client.post(
            "/api/v1/loans/return/initiate",
            json={"aztec_code": asset.aztec_code, "kiosk_id": str(_VALID_KIOSK_ID)},
...
def test_return_initiate_returns_400_when_no_active_loan(client_with_overrides):
    ...
    fake_db = _QueuedSession(student, [], SimpleNamespace(), asset, None)
    with client_with_overrides(fake_db) as client:
        response = client.post(
            "/api/v1/loans/return/initiate",
            json={
...
def test_return_initiate_returns_503_when_no_locker_available(client_with_overrides):
    ...
    fake_db = _QueuedSession(
        student, [], SimpleNamespace(), asset, active_loan, active_loan, None
    )
    with client_with_overrides(fake_db) as client:
        response = client.post(
            "/api/v1/loans/return/initiate",
...
def test_return_initiate_returns_409_on_loan_lock_contention(client_with_overrides):
    ...
    locking_db = _LoanLockAt6(student, [], SimpleNamespace(), asset, active_loan)
    with client_with_overrides(locking_db) as client:
        response = client.post(
            "/api/v1/loans/return/initiate",
            json={
...
def test_return_initiate_returns_409_when_loan_state_changed_concurrently(
    ...
    fake_db = _QueuedSession(student, [], SimpleNamespace(), asset, active_loan, None)
    with client_with_overrides(fake_db) as client:
        response = client.post(
            "/api/v1/loans/return/initiate",
            json={
...
def test_return_initiate_returns_409_for_duplicate_idempotency_key(
    ...
    fake_db = _QueuedSession(
        student, [], SimpleNamespace(), asset, loan, loan, free_locker, student
    )
